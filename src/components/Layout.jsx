...
const Layout = () => {
    ...
    return (
        <div className="min-h-screen bg-[var(--color-bg)] flex flex-col">
            {/* Offline Banner */}
            {isOffline && (
                <div className="sticky top-0 z-50 w-full bg-amber-500/10 backdrop-blur-md border-b border-amber-500/20 py-1.5 px-4 flex items-center justify-center gap-2 animate-slide-down">
                    <FiWifiOff size={12} className="text-amber-600 animate-pulse" />
            ...
            <div className="flex-1 flex">
                {/* Desktop Sidebar */}
                <aside className={`hidden lg:flex flex-col ${sidebarWidth} bg-[var(--color-surface)] border-r border-[var(--color-border-light)] fixed ${isOffline ? 'top-[33px]' : 'top-0'} bottom-0 z-30 transition-all duration-300`}>
                    {/* Greeting + Toggle */}
                ...
                <div className={`flex flex-col min-h-screen flex-1 ${mainMargin} transition-all duration-300`}>
                    {/* Mobile Header */}
                    <header className={`lg:hidden sticky ${isOffline ? 'top-[33px]' : 'top-0'} z-20 bg-[var(--color-nav-bg)] backdrop-blur-xl border-b border-[var(--color-border-light)] shadow-[0_1px_3px_var(--color-shadow)]`}>
